...
#define LAUNDR_VERSION "5.56"
#define LAUNDR_CODENAME "BackdoorBuster Thunder"  // 5.56 = Fixed: stop emulation before freeing NFC to prevent listener crash
...
typedef struct {
    ...
    uint32_t transaction_count;    // Number of completed transactions
...
static void laundr_start_emulation(LaundRApp* app) {
    ...
    // Show LIVE emulation screen with UID and transaction stats
    widget_reset(app->widget);
    ...
    snprintf(app->widget_str1, sizeof(app->widget_str1),
             "UID: %02X%02X%02X%02X", current_uid[0], current_uid[1], current_uid[2], current_uid[3]);
    widget_add_string_element(
    ...
    widget_add_string_element(
        app->widget, 2, 44, AlignLeft, AlignTop, FontSecondary, "Waiting for reader...");
    widget_add_string_element(
